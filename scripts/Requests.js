...
export const Requests = () => {
  ...
  let liArrayIncomplete = incompleteRequests.map((request) => {
    return `
        ...
        <div class = "left-area">
            Reservation for ${request.childName} on ${
      request.dateOfParty
    } for ${request.lengthOfReservation}
        </div>
        ...
        <div class = "right-area" >
          <button class="request__deny" id="request--${
            request.id
          }">deny</i></button>
    </div>
        </li>
  ...
  let liArrayComplete = completedRequests.map((request) => {
    ...
    return `
        ...
        <div class = "right-area" >
          <button class="request__save" id="request--${request.id}">save</button>
    </div>
        </li>
        `;
  });
...
mainContainer.addEventListener("change", (event) => {
  if (event.target.id === "clowns") {
    ...
    const completion = {
      //modern JS notation for when the name of the property is the same as the name of the variable holding the value for the property
      requestId,
      clownId,
      date_created,
    };
